...
def Interpolator(img_path, mask_path, random_ratio, upscale_ratio, flag):
    ...
    if flag == 2:
        ...
        cv2.imwrite('./Orignal.bmp', img)
        cv2.imwrite('./Masked.bmp', masked)
        cv2.imwrite('./RBF_GAUSSIAN.bmp', ans_RBF_GAUSSIAN)
        cv2.imwrite('./RBF_TPS.bmp', ans_RBF_TPS)
        cv2.imwrite('./NNI.bmp', ans_NNI)
        cv2.imwrite('./BNI.bmp', ans_BNI)
