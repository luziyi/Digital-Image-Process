...
def DoG(src, ksize, sigma, k, thresh=None, alpha=0.01):
    ...
    k1_img = cv2.filter2D(src=src, ddepth=cv2.CV_32FC1, kernel=kernel_1)
    k2_img = cv2.filter2D(src=src, ddepth=cv2.CV_32FC1, kernel=kernel_2)
    ...
    return edge_image, (k1_img, k2_img)