...
def refine_flow(flow, mag, ksize):
    ...
    mag_neighbors = find_neighbors(mag, ksize, s=1, out_h=h, out_w=w)
...
def detect_edge(img, flow, thresh, sigma_c, rho,
    ...
    gauss_s = make_gauss_filter(sigma_c * 1.6, threshold=thresh)
    ...
    img = np.pad(img, ((w_fdog, w_fdog), (w_fdog, w_fdog))) 
...
def run(img, sobel_size=5, etf_iter=4, etf_size=7,
    ...
    absX = cv.convertScaleAbs(flow[0])
    absY = cv.convertScaleAbs(flow[1])
    edge_sobel = cv.addWeighted(absX, 0.5, absY, 0.5, 0)
    edge_sobel[edge_sobel != 0] = 255
    cv.imwrite('flow_0.jpg', edge_sobel)
    ...
    for i in range(etf_iter):
        ...
        edge_sobel = cv.addWeighted(absX, 0.5, absY, 0.5, 0)
        edge_sobel[edge_sobel != 0] = 255
        cv.imwrite(f'flow_{i+1}.jpg', edge_sobel)
